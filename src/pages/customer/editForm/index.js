...
export default class EditModal extends PureComponent {
  ...
  onSave = () => {
    ...
    const { record, khxzvalue, khmchasError, qyjchasError, gswzhasError, shbmhasError, zcmhasError, nsdjhhasError, ygrshasError, zjlhasError, mjhasError, emailhasError, ywzbrhasError } = this.state;
    ...
    validateFields((errs, fields) => {
      ...
      // 客户类别
      _.set(fields, 'khlb', _.join(fields.khlb, '/'));
      // 客户性质
  ...
  render() {
    ...
    const { record, khxzOptions, qyType, isShow, mjhasError, ywzbrhasError, emailhasError, zjlhasError, ygrshasError, nsdjhhasError, zcmhasError, shbmhasError, gswzhasError, khmchasError, qyjchasError } = this.state;
    ...
    return (
      <div>
        ...
        <Picker
          ...
          {...getFieldProps('khxzl', {
            initialValue: record && record.khxz && [record.khxz] || ['1'],
          })}
          disabled
          data={khxz}
          cols={1}>
        ...
        <Picker
          onOk={value => this.getValue(value)}
          {...getFieldProps('khlb', {
            initialValue: record.khlb && record.khlb.split('/').map((item) => {
              return Number(item);
            }),
          })}
          data={khlbOptions}>
        ...
        <Accordion accordion defaultActiveKey="0">
          ...
          <Panel header='其他' style={{marginBottom: 30}}>
            <InputItem
              clear
